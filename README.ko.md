...
**방 성장 시스템**
- 최근 30일간의 작업 히스토리를 추적
- 특정 카테고리 비중이 높아지면 방이 업그레이드 (예: 코딩 40% 이상 → 듀얼 모니터 → 트리플 스크린)
- 5가지 업그레이드 경로: 코딩, 쇼핑, 글쓰기, 조사, 일정
...
│   │   └── room-growth.ts     # 작업 히스토리 기반 방 업그레이드
...
└── data/                # 런타임 데이터 (카테고리, 작업 히스토리, 방 업그레이드)
```
...
| 카테고리 | 레벨 1 | 레벨 2 |
|----------|--------|--------|
| 코딩 | 40% → 듀얼 모니터 | 80% → 트리플 스크린 |
| 쇼핑 | 25% → 장바구니 확장 | 50% → 배송박스 더미 |
| 글쓰기 | 30% → 타자기 | 60% → 책장 확장 |
| 조사 | 30% → 돋보기 | 60% → 연구 보드 |
| 일정 | 30% → 디지털 달력 | 60% → 플래너 데스크 |
